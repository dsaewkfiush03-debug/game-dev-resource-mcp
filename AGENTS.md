...
14. Coverage benchmarks must reuse the same conservative license/search filters as normal recommendations. Never improve benchmark scores by accepting unknown/non-commercial licenses or changing default share-alike posture.
15. Do not hide weak coverage by removing difficult benchmark scenarios, excluding required slots with no providers, lowering the depth target below three, or counting discovery-only registry entries as successful provider candidates.
16. Treat live-provider errors as a separate reliability signal; do not silently convert an API/network failure into either a successful result or a permanent catalog gap.
17. Automatic downloads require an explicit trusted HTTPS host allowlist and provider-backed file metadata.
18. Never automatically execute, extract or install downloaded third-party content.
19. Never guess a download URL merely to make a workflow automatic.
20. Local installation must remain contained inside an explicit absolute project root.
